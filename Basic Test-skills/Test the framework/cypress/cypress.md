...
cypress的优势在于可以同时进行UI自动化和接口自动化，比如测试一个界面，某些对象很难定位。一个界面需要登录，需要下订单，登录界面的识别对象非常复杂，还会经常发生变化，这个时候需要花很多时间写登录脚本，但是最后测试的点还不在登录上面。浪费时间，最后测试的关键在于下订单——————换成cypress，可以做接口，在做登录的时候，不需要定位对象，不需要找到用户名，密码框，登录按钮，只需要找到，点登录的时候，向服务器端发送的登录请求就行，将其当做接口请求，请求成功后，拿到token和session，传给下订单这个业务，下订单的时候只是需要登录状态，登录状态的保持既可以通过界面做操作，也可以通过接口做操作。
